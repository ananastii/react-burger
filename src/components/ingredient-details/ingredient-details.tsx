...
const IngredientDetails = () => {
  ...
  const { id } = useParams();
  const [ingredientData, setIngredientData] = useState<TIngredientInfo>(ingredient);

  useEffect(() => {
    if (ingredient) {
      setIngredientData(ingredient);
    } else {
      setIngredientData(ingredients.find((item: TIngredientData) => item?.info._id === id)?.info)
    }
