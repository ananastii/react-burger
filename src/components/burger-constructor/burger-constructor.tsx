...
import { TIngredientInfo, TIngredientData, TIngredientFilling } from '../../services/types/data';
...
const BurgerConstructor = () =>  {
  ...
  const closeOrderModal = (orderFailed: boolean) => {
    ...
    if (!orderFailed) {
      ...
      ingredients.filter((item: TIngredientData) => item.qty > 0)
        .forEach((item: TIngredientData) => dispatch(setCount(item.info._id, 0)));
    }
  };

  let totalPrice = useMemo(() => fillings.reduce(
      (price: number, item: TIngredientData) => (price += item.info.price),
